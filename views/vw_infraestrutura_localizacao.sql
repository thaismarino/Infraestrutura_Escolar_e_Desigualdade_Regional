...
CREATE OR REPLACE VIEW vw_infraestrutura_localizacao AS
SELECT
  CASE TP_LOCALIZACAO
    WHEN 1 THEN 'Urbana'
    WHEN 2 THEN 'Rural'
  ...
  ROUND(SUM(CASE WHEN IN_AGUA_REDE_PUBLICA = 1 THEN 1 ELSE 0 END) / NULLIF(COUNT(*),0), 4) AS pct_agua_sim,
  ...
  ROUND(SUM(CASE WHEN IN_ESGOTO_REDE_PUBLICA = 1 THEN 1 ELSE 0 END) / NULLIF(COUNT(*),0), 4) AS pct_esgoto_sim,
  ...
  ROUND(SUM(CASE WHEN IN_ENERGIA_REDE_PUBLICA = 1 THEN 1 ELSE 0 END) / NULLIF(COUNT(*),0), 4) AS pct_energia_sim,
  ...
  ROUND(SUM(CASE WHEN IN_LIXO_SERVICO_COLETA = 1 THEN 1 ELSE 0 END) / NULLIF(COUNT(*),0), 4) AS pct_lixo_sim
